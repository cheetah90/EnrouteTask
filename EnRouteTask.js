...
//These are the objects that need to be cleaned up before each run
var stepIter;
var routeIter;
var legIter;
...
function initialize() {
	...
	//Add the panel that displays the turn by turn instructions, and the overall travelling time
	directionsDisplay.setPanel(document.getElementById('directions-panel'));
	
	//Initiate a placeService Object for the Place Search, see later
...
//The Main function of Route Calculation, called when the user clicks "Submit"
function calcRoute(){
	//If the marker on the map is already created, delete is
	if(marker!=null)
	{
		marker.setMap(null);
		marker=null;
	...
	directionsService.route(firstdirectonRequest, function(directionsResult, status){
		if (status==google.maps.DirectionsStatus.OK){
			//In the Google maps API, directionsResult would be the object that holds everything about the route. 
			//We are gonna parse directionsResult to iterate each turn.
...
function IterateTurn(directionsResult){
	...
	//Get the START LOCATION of the route and search for nearby establishments of the selected type
	//We search the start location seperately because the start_location does not fit the general patterns of the other turns
	var searchCoord;			
	searchCoord=directionsResult.routes[routeIter].legs[legIter].start_location; 
	//The way that Google Maps organizes their direction result is as follows:
	// route[]  (which holds a set of alternative routes, if any)
	//		--legs[]	(which holds a set of routes between each pair of waypoints, if any)
	//				--step[]	(which contains the turns in turn-by-turn direction)
	

	//Construct the placeSearchRequest Object, which searches for the establishments of the selected type near the Starting_Point. The results are ranked by distance to the search location 
	placesSearchRequest = {
...
function searchNearestEstablishment(placesSearchRequest,directionsResult){
	/*This is a recursive function, in the sense that it will call it self iteratively 
	until the place search for establishments of the selected type at each turn has finished. 
	The reason for using a recursive function, rather than a straightforward for-loop, is that 
	Google's API javascript relies on a callback function to process the returned objects of Place Service 
	and Direction Service queries, and as a result, any code after the Place Service function and 
	Direction Service will actually be processed before the place search result and direction 
	search (routing) result are returned. 
	*/
	...
	placesService.nearbySearch(placesSearchRequest, function(placesResults, status){
		...
		{	
			...
			updatePlaceResultObject(tp_nearestEstablishmentObject,placesResults[0]); //Since the establishments of selected type at specific location is ranked by distance, the first one is the nearest one.
			...
			directionsService.route(compareDirectonRequest, function(compareDirectionsResult, status)
			{
				if (status==google.maps.DirectionsStatus.OK)
				{
					...
					//If we finish the place search at every turn, create a marker at this recorded nearest establishment and do the final routing.
					if(routeIter>=directionsResult.routes.length)
					{
						createMarker(nearestEstablishmentObject);
						finalDirectionsServiceandRender();
					}
					else	//if we have not finished places searches at every turn, use the next turn in the original route for a nearest establishment search
					{
...
//This is the final Direction Service, which use the original start location and original end location from user input, and the nearest establishment which has the minimum detour time as a waypoint.
...
//Create a Marker for the nearest establishment with minimum detour time.
